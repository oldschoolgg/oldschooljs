...
test('Master Clues', t => {
	t.plan(2);
	const masterCasketOpened = Clues.Elite.openCasket();

	t.notOk(masterCasketOpened.length > 6, "Asserted master casket doesn't have more than 7 items");

	t.notOk(masterCasketOpened.length < 4, "Asserted master casket doesn't have less than 5 items");
});
